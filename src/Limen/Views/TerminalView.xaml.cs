...
public partial class TerminalView : UserControl, IDisposable
{
    ...
    public async void FocusTerminal()
    {
        ...
        try { await Browser.ExecuteScriptAsync("window.terminalFocus && window.terminalFocus()"); }
    ...
    {
        if (_shell is not null || _disposed || _connecting) return;
        _connecting = true;
    ...
    {
        // A private OSC carries the literal path; OSC 7 remains reserved for
        // URI-encoded reports from the user's own shell integration.
        const string command =
            " if [ -n \"$BASH_VERSION\" ]; then " +
            "__ssb_cwd(){ printf '\\033]777;limen-cwd;%s\\033\\\\' \"$PWD\"; }; " +
            "if declare -p PROMPT_COMMAND 2>/dev/null | grep -q 'declare -a'; then " +
            "PROMPT_COMMAND=(__ssb_cwd \"${PROMPT_COMMAND[@]}\"); else " +
    ...
    private async Task WriteAsync(byte[] bytes)
    {
        ...
        try
        {
            if (_disposed || !ReferenceEquals(shell, _shell)) return;
            await Task.Run(() =>
            {
    ...
    public void Dispose()
    {
        ...
        _route?.Dispose();
        _route = null;
        // Pending writers still release this semaphore after the shell closes.
        Browser.Dispose();
    }
}
